...
class MosqueApp:
    def __init__(self, root):
        ...
        # تحديد الثيم الحالي بناءً على الوقت
        ...
        if os.path.exists(bg_file):
            try:
                ...
                # خلفية حسب الثيم
                bg_color = self.theme_colors[self.current_theme]["bg_color"]
                self.bg_label = ctk.CTkLabel(root, text="", fg_color=bg_color)
                self.bg_label.place(x=0, y=0, relwidth=1, relheight=1)
        else:
        ...
        # Main container فوق الخلفية
        main_container = ctk.CTkFrame(root, fg_color="transparent")
        main_container.pack(fill="both", expand=True, padx=50, pady=30)  # إضافة هوامش
        ...
        # إزالة pack_propagate(False) للسماح بالتوسع
        ...
        # Mosque & place labels
        text_color = self.theme_colors[self.current_theme]["text_color"]
        title = self.theme_colors[self.current_theme]["title"]
        ...
        self.mosque_label.pack(pady=(40, 10))
        ...
        self.place_label.pack(pady=(10, 5))
        ...
        prayer_bg = self.theme_colors[self.current_theme]["prayer_bg"]
        ...
        next_prayer_frame.pack(pady=30, expand=True, fill="both")
        # إزالة pack_propagate(False) للسماح بالتوسع
        ...
        self.next_title.pack(pady=(30, 10))
        ...
        self.next_name.pack(pady=20)

        countdown_color = self.theme_colors[self.current_theme]["countdown_color"]
        ...
        self.countdown.pack(pady=(10, 30))

        # Schedule - MODERN COLUMNS DESIGN
        schedule_bg = self.theme_colors[self.current_theme]["schedule_bg"]
        schedule_frame = ctk.CTkFrame(
            main_container,
            fg_color=schedule_bg,
            corner_radius=25,
            height=self.font_scales['schedule_height']
        )
        schedule_frame.pack(side="bottom", pady=30, fill="x")
        # إزالة pack_propagate(False) للسماح بالتوسع
        ...
        columns_container.pack(pady=15, expand=True)
        ...
        for i, prayer in enumerate(prayers):
            ...
            column_bg = self.theme_colors[self.current_theme]["column_bg"]
            column_border = self.theme_colors[self.current_theme]["prayer_border"]
            ...
            column_frame = ctk.CTkFrame(
                ...
                corner_radius=15,
                border_width=2,
                border_color=column_border
            )
            column_frame.grid(row=0, column=reverse_i, padx=15, pady=5, sticky="nsew")
            # إزالة grid_propagate(False) و pack_propagate(False)
            ...
            name_label.pack(pady=(20, 5), expand=True)
            ...
            time_label.pack(pady=(5, 20), expand=True)
            
            self.prayer_columns.append((prayer, time_label))

        # جعل الأعمدة تتوسع بشكل متساوٍ
        ...
        self.overlay_countdown.pack(pady=20)
    ...
    def calculate_font_scales(self, screen_width, screen_height):
        ...
        if screen_width >= 3840:  # TV 50 inch أو أكبر
            scale_factor = 2.5  # زيادة المعامل للتلفاز الكبير
            print(f"تم التعرف على تلفاز 50 بوصة، معامل القياس: {scale_factor}")
        ...
        return {
            'mosque_font': int(60 * scale_factor),
            'place_font': int(50 * scale_factor),
            'date_font': int(45 * scale_factor),
            'next_title_font': int(50 * scale_factor),
            'next_name_font': int(65 * scale_factor),
            'countdown_font': int(55 * scale_factor),
            'schedule_title_font': int(50 * scale_factor),
            'prayer_name_font': int(35 * scale_factor),
            'prayer_time_font': int(32 * scale_factor),
            'overlay_icon_font': int(150 * scale_factor),
            'overlay_text_font': int(80 * scale_factor),
            'overlay_countdown_font': int(90 * scale_factor),
            'button_height': int(80 * scale_factor),
            'next_prayer_width': int(900 * scale_factor),
            'next_prayer_height': int(350 * scale_factor),
            'schedule_height': int(280 * scale_factor),
            'column_width': int(180 * scale_factor),
            'column_height': int(140 * scale_factor)
        }
    ...
    def get_current_theme(self):
        ...
        # النهاري: من 6 صباحاً إلى 6 مساءً
        # الليلي: من 6 مساءً إلى 6 صباحاً
        if 6 <= hour < 18:
